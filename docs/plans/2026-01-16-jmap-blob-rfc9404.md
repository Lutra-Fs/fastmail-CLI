# JMAP Blob Support Implementation Plan

> **IMPORTANT:** All blob commands implemented below use the JMAP Blob Management Extension (RFC 9404), which is **not supported by Fastmail**.

> **Fastmail Support Status:**
> - **Not Supported (RFC 9404):** All `Blob/*` methods including `Blob/upload`, `Blob/get`, `Blob/lookup`
> - **Internal Use Only (RFC 8620):** Fastmail uses `uploadUrl`/`downloadUrl` internally for email attachments, but the CLI blob commands do not currently use these URLs

This implementation provides full RFC 9404 support for compatibility with other JMAP providers. On Fastmail, none of the `fastmail blob` commands will work.
